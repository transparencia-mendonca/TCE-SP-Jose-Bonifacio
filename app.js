...
const DBN="tcesp-jose-bonifacio-v6",STORE="dados",KEY="base";
...
function tceExpenseUrl(year){return `https://transparencia.tce.sp.gov.br/municipio/jose-bonifacio/${year}/despesas`}
...
function auditExcludedSupplier(row){let t=normalizedName(row);return /FOLHA DE PAGAMENTO|PAGAMENTOS[- ]SERVIDORES|ENERGISA|TELEFONICA|TELECOM|CORREIOS|TRIVALE|ROM CARD|INSS|INSTITUTO NACIONAL DO SEGURO SOCIAL|CAIXA ECONOMICA|BANCO DO BRASIL|CAMARA MUNICIPAL|PREFEITURA MUNICIPAL|MUNICIPIO DE JOSE BONIFACIO/.test(t)}
...
$("#backupBtn").onclick=()=>download(`tcesp-jose-bonifacio-backup-${new Date().toISOString().slice(0,10)}.json`,JSON.stringify(db));$("#restore").onchange=async e=>{let x=JSON.parse(await e.target.files[0].text());if(!x.despesas||!x.receitas)return alert("Backup inválido");db=x;await save();render();alert("Backup restaurado.")};$("#resetBtn").onclick=async()=>{if(confirm("Restaurar a base original do repositório e apagar apenas as atualizações locais posteriores?")){db=structuredClone(window.SEED_DATA);await save();render()}};
...
const MUNICIPIO_API="jose-bonifacio";
...
(async()=>{try{
  db=await idbGet();
  if(!db){let old=localStorage.getItem("tcesp-jose-bonifacio-data")||localStorage.getItem("tcesp-data");db=old?JSON.parse(old):structuredClone(window.SEED_DATA);}
  // Migração conservadora: a atualização da interface não apaga exercícios já carregados pela API.
  // O seed existente no repositório é usado apenas para preencher estruturas ausentes.
  if(!db||typeof db!=="object")db=structuredClone(window.SEED_DATA);
  if(!Array.isArray(db.despesas))db.despesas=structuredClone(window.SEED_DATA?.despesas||[]);
  if(!Array.isArray(db.receitas))db.receitas=structuredClone(window.SEED_DATA?.receitas||[]);
  if(!Array.isArray(db.exercicios))db.exercicios=[];
  db.exercicios=[...new Set([...(window.SEED_DATA?.exercicios||[]),...db.exercicios,...db.despesas.map(x=>+x.ano),...db.receitas.map(x=>+x.ano),2020,2021,2022,2023,2024,2025,2026])].filter(Boolean).sort((a,b)=>a-b);
  if(!db.apiUpdates)db.apiUpdates={};if(!db.autoCheck)db.autoCheck={};
  await save();localStorage.removeItem("tcesp-jose-bonifacio-data");
}catch(e){db=structuredClone(window.SEED_DATA||{despesas:[],receitas:[],exercicios:[2020,2021,2022,2023,2024,2025,2026]});console.warn(e)}
