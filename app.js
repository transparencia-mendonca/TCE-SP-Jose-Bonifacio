...
(async()=>{try{
  db=await idbGet();
  if(!db){
    let old=localStorage.getItem("tcesp-jose-bonifacio-data")||localStorage.getItem("tcesp-data");
    if(old){
      db=JSON.parse(old);
    }else{
  ...
  await save();
  localStorage.removeItem("tcesp-jose-bonifacio-data");
}catch(e){
